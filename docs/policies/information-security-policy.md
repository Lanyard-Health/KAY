...
| E-9 | ~~The master encryption key exists only in the hosting provider's environment configuration~~ **Partially remediated 2026-08-08.** A verified Bitwarden copy now exists (`docs/key-custody-runbook.md`, fingerprint match confirmed). Still outstanding: the Deputy share and an independent copy held outside Bitwarden. | Reduced. A Bitwarden account lockout would still take the only surviving copy | Complete Steps 4 and 4b of the key custody runbook | Deputy share + sealed physical copy `[BLANK — Kay]` |
| E-10 | Object storage has no independent backup (Business Continuity Plan §3.3). R2 supports neither versioning nor native replication, and bucket locks are incompatible with the application's delete paths and with §13.2. | Loss of the bucket loses every uploaded document | Scheduled copy job to a second bucket — see BCP §3.3 | `[BLANK — Kay]` |
