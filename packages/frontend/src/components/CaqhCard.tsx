...
export function CaqhCard({ providerId, provider }: CaqhCardProps) {
  const [isEditingCredentials, setIsEditingCredentials] = useState(false);
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);
  const [activeTab, setActiveTab] = useState<'status' | 'credentials'>('status');
      ...
      <div className="border-b border-gray-200">
        <nav className="flex -mb-px">
          <button
            ...
            className={`flex-1 py-2 px-4 text-xs font-medium text-center border-b-2 ${
              activeTab === 'status'
            ...
            {provider.caqhProviderId ? (
              <>
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-medium text-gray-900">{provider.caqhProviderId}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-500">Status</span>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium capitalize ${
                    provider.caqhStatus === 'active' ? 'bg-green-100 text-green-800' :
                    provider.caqhStatus === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                    provider.caqhStatus === 'expired' ? 'bg-red-100 text-red-800' :
                    'bg-gray-100 text-gray-800'
                  }`}>
                    {provider.caqhStatus || 'Unknown'}
                  </span>
                </div>
                {provider.caqhLastSync && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm text-gray-900">
                      {format(new Date(provider.caqhLastSync), 'MMM d, yyyy')}
                    </span>
                  </div>
                )}
                <div className="pt-2">
              ...
              <div className="text-center py-4">
                <p className="text-sm text-gray-500 mb-3">Not connected to CAQH</p>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 pr-10"
