...
interface CaqhCredentialsCardProps {
  providerId: string;
}

export function CaqhCredentialsCard({ providerId }: CaqhCredentialsCardProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [showPassword, setShowPassword] = useState(false);

  const { data: statusData, isLoading } = useCaqhCredentialStatus(providerId);
    ...
    <div className="bg-white rounded-lg shadow">
      <div className="px-4 py-3 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-medium text-gray-900">CAQH ProView Credentials</h3>
          <a
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                  status ? getCredentialStatusColor(status) : 'bg-gray-100 text-gray-800'
                }`}
              >
                {status ? getCredentialStatusLabel(status) : 'Unknown'}
              </span>
            </div>
            ...
            {status?.hasCredentials && (
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-gray-900">{status.username}</span>
              </div>
            )}
