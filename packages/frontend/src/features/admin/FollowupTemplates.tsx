...
import { useFollowUpTemplates, useCreateFollowUpTemplate } from '../../hooks/useFollowupTemplates';
...
export default function FollowupTemplates() {
  ...
  const { data: templates, isLoading } = useFollowUpTemplates(filters);
    ...
    createMutation.mutate(
      ...
      },
      {
        onSuccess: (data) => {
          ...
          setNewDescription('');
          setNewPayerTrackId('');
          navigate(`/admin/followup-templates/${data.id}`);
        },
        onError: () => {
      ...
      <div>
        <div className="sm:flex sm:items-center sm:justify-between mb-8">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Manage follow-up workflow templates</p>
          </div>
        </div>
        <div className="card overflow-hidden">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50/80">
              <tr>
                {['Name', 'Payer Track', 'Version', 'Status', 'Steps', 'Runs', 'Last Updated'].map((h) => (
      ...
      <div>
        ...
        {!templates || templates.length === 0 ? (
          <EmptyState
            ...
            title="No follow-up templates found"
            description={
              activeTab !== 'all'
                ? `No ${activeTab} templates. Try a different filter.`
                : 'Get started by creating your first follow-up template.'
            }
            action={
              activeTab === 'all'
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {templates.map((template) => {
                  ...
                  return (
                    <tr
                      key={template.id}
                      onClick={() => navigate(`/admin/followup-templates/${template.id}`)}
