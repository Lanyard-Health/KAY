...
const REASON_OPTIONS = [
  ...
  { value: 'Left practice', label: 'Left practice' },
...
/**
 * Confirmation modal for soft-deleting a provider. Reason capture is fully optional
 * (per Amendment 2): the Delete button is enabled with no selection. We send the
 * combined "<reason> — <notes>" string (skipping the dropdown when 'Other') so a
 * single deletionReason column captures both signals.
 *
 * Mirrors the destructive styling of TerminationConfirmDialog so the visual cue is
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="relative z-10 inline-block w-full max-w-md p-6 my-8 text-left align-middle bg-white rounded-2xl shadow-xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-medium text-gray-900">Delete this provider?</h3>
          </div>

          <p className="text-sm text-gray-700 mb-4">
            <span className="font-semibold">{providerName}</span> will be hidden from active lists
            and queues. The record stays in the system for our records and can be restored later.
          </p>

          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="delete-reason">
            Reason
          </label>
          <select
            id="delete-reason"
          ...
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="delete-notes">
            Notes (optional)
          </label>
          <textarea
            id="delete-notes"
