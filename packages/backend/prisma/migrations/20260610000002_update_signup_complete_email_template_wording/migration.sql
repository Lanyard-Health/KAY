-- Updates the SIGNUP_COMPLETE welcome email wording to introduce Kay by name
-- and add "As the founder," lead-in to the credentialing-pain paragraph.
--
-- Idempotent: the UPDATE matches by trigger_event; safe to re-run.
-- No-op if the template row doesn't exist yet.

UPDATE email_templates
SET
  body = $WELCOME_HTML$<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
<body style="margin:0; padding:0; background-color:#f4f6f5; font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Inter,Roboto,Helvetica,Arial,sans-serif; color:#1a2e26;">
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <p style="margin:0 0 16px 0; font-size:16px; line-height:1.6; color:#1a2e26;">
                <strong>{{practiceName}}</strong> is set up and ready. I'm Kay Ward, the founder of Lanyard Health — and I'm glad you're here.
              </p>

              <p style="margin:0 0 16px 0; font-size:16px; line-height:1.6; color:#1a2e26;">
                As the founder, I've lived the credentialing pain alongside other practice owners — the forms, the back-and-forth with payers, the silent rejections, the months of lost revenue waiting on a single signature. That's why I built Lanyard: to take that work off your plate and give you the visibility you should've had the whole time.
              </p>
  ...
  updated_at = NOW()
WHERE trigger_event = 'SIGNUP_COMPLETE';
