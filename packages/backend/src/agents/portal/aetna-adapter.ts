...
const MOCK_AETNA_BASE_URL =
  process.env['MOCK_AETNA_BASE_URL'] ?? 'http://localhost:3002/mock-aetna';
...
async function acquireLock(): Promise<void> {
  ...
  if (waitQueue.length >= MAX_QUEUE_DEPTH) {
    throw new Error('Aetna submission busy, try again later');
  }
  return new Promise<void>((resolve, reject) => {
    const timer = setTimeout(() => {
      const idx = waitQueue.findIndex((w) => w.resolve === resolve);
      if (idx !== -1) waitQueue.splice(idx, 1);
      reject(new Error('Aetna submission timed out waiting for availability'));
    }, QUEUE_TIMEOUT_MS);
    waitQueue.push({
...
export class AetnaAdapter implements PayerAdapter {
  readonly adapterType = 'aetna_demo';
  ...
  async submit(input: SubmissionInput): Promise<PayerAdapterResult> {
    ...
    try {
      ...
      logger.info('Launching Aetna demo browser submission', {
      ...
      page.on('dialog', async (dialog) => {
        logger.info('Auto-dismissing browser dialog during Aetna demo', {
          type: dialog.type(),
      ...
      logger.info('[aetna-demo] navigating to login page');
      await page.goto(`${MOCK_AETNA_BASE_URL}/login.html`, {
        waitUntil: 'domcontentloaded',
        timeout: 20_000,
      });
      await page.waitForSelector('#username', { timeout: 10_000 });
      logger.info('[aetna-demo] typing credentials');
      await page.type('#username', username, { delay: 40 });
      await page.type('#password', password, { delay: 40 });
      logger.info('[aetna-demo] clicking login');
      await Promise.all([
        page.click('#login-button'),
        page.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 20_000 }),
      ]);

      logger.info('[aetna-demo] on dashboard, clicking submit-enrollment');
      ...
      logger.info('[aetna-demo] on enrollment form, filling fields');
      ...
      logger.info('[aetna-demo] submitting enrollment');
      await Promise.all([
      ...
      logger.info('Aetna demo submission completed', {
        workflowId: input.workflowId,
        confirmationNumber,
      });
      ...
      logger.error('Aetna demo submission failed', {
        workflowId: input.workflowId,
        error: message,
      });
...
async function typeIfPresent(page: Page, selector: string, value: string): Promise<void> {
  if (!value) {
    logger.info(`[aetna-demo] skip ${selector} (empty value)`);
    return;
  }
  const el = await page.$(selector);
  if (!el) {
    logger.info(`[aetna-demo] skip ${selector} (not on page)`);
    return;
  }
  const inputType = await page.$eval(selector, (node) => (node as HTMLInputElement).type ?? 'text');
  logger.info(`[aetna-demo] fill ${selector} (type=${inputType}) <- ${value.slice(0, 30)}`);
  if (inputType === 'date') {
