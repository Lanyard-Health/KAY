...
import payerContactInfoRoutes from './routes/payer-contact-info.routes.js';
...
import practiceInvitationRoutes from './routes/practiceInvitations.routes.js';
import emailRoutes from './routes/email.routes.js';
import { enrollmentStrategyRoutes } from './routes/enrollmentStrategy.routes.js';
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (!origin) {
      ...
      callback(null, true);
    } else if (process.env['NODE_ENV'] !== 'production' && /^https?:\/\/[A-Za-z0-9-]+(\.[A-Za-z0-9-]+)*\.(replit\.dev|repl\.co)(:\d+)?$/.test(origin)) {
      // In dev, allow Replit preview domains (which have multi-level
      // subdomains, e.g. <id>.<cluster>.replit.dev)
      callback(null, true);
    } else {
...
app.use('/api/v1/practices', practiceInvitationRoutes);
app.use('/api/v1/practices', practiceRoutes);
app.use('/api/v1/email', emailRoutes);
app.use('/api/v1/enrollment-strategy', enrollmentStrategyRoutes);
